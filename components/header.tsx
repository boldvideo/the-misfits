...
export function Header({ logo, logoDark, menuItems }: HeaderProps) {
  ...
  return (
    <>
      <header
        className={
          `px-5 lg:px-[2vw] py-[1vw] transition-all ` +
          ...
          <nav className="flex flex-col lg:flex-row gap-4 lg:gap-0">
            <div className="flex items-center justify-between w-full">
              <div className="flex items-center">
                ...
                <div
                  className={`${spaceGrotesk.className} hidden uppercase lg:flex space-x-1`}
                >
                  {menuItems.map((item, idx) => (
                    <Link
                      className="text-sm mr-[1vw] py-2 rounded-md hover:underline transition-colors"
                      key={`${item.label}-${idx}`}
                      href={item.url}
                    >
                      {item.label}
              ...
              <div className="hidden lg:flex items-center space-x-[2.5vw] flex-1 justify-end max-w-md">
                <Link
                  href="https://www.instagram.com/wewantmisfits/"
                  target="_blank"
                ...
                <Suspense>
                  <SearchBar
                    className={`${spaceGrotesk.className} w-full max-w-2xs`}
                  />
                </Suspense>
              </div>
